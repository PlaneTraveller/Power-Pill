...
for key in timeline:
    ...
    for i in range(0, len(new_confirmed_cases)):
        ...
        new_confirmed_cases_columns = pandas.DataFrame.from_dict(
            new_confirmed_cases_date, orient=('index'))
        new_deaths_columns = pandas.DataFrame.from_dict(new_deaths_date,
                                                        orient=('index'))
        new_deaths_columns = new_deaths_columns.drop(index='dt')
        columns = pandas.concat(
            [new_confirmed_cases_columns, new_deaths_columns])
