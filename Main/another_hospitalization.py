...
for key in state:   #go through every element
    hospitalization=requests.get('https://jhucoronavirus.azureedge.net/api/v1/timeseries/us/hospitalization/{}.json'.format(key))   #fill the .json name automatically
    hospitalization=hospitalization.json()  #change the .json into a list whose element number is one
    hospitalization=hospitalization[0]  #change the list into a dict
    data=pandas.DataFrame.from_dict(hospitalization,orient=('index'))   #change the dict into .csv
    data.to_csv('C:/Git/Power-Pill/Main/output/{}_hospitalization.csv'.format(state[key])) #generate a .csv whose name can be filled with the state name automatically
